...
const {user} = useContext(AppContext)
  ...
  return (
    ...
    className='min-h-[80vh] text-center pt-14 mb-10'>
      ...
      rounded-full mb-6 c'>Our Plans</button>
      ...
      <div className='flex flex-wrap gap-6 justify-center text-left'>
        {plans.map((item,index)=>(
          <div key={index}
          className='bg-white drop-shadow-sm border-0 rounded-lg py-12
          px-8 text-gray-600 hover:scale-105 transition-all duration-500'>
            ...
            <button className='bg-gray-800 rounded-md text-white 
            w-full mt-8 text-sm py-2.5 min-w-52'>{user? "Purchase" : "Get Started"}</button>
          </div>
        ))}
      </div>
